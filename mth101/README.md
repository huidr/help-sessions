...
for any integer values of $a_1, \ldots, a_k$. Conversely, any number of this form is an element of $H$. Note that $a_i x_i = x_i + \cdots + x_i$ is the $a_i$-th power of $x_i$ which would otherwise be written as ${x_i}^{a_i}$ in a multiplicative group. Indeed, in a multiplicative group, the representation above takes the form ${x_1}^{a_1}{x_2}^{a_2}\cdots{x_k}^{a_k}$. By an important result from number theory, the representation above can also be written as $ax$ where $x = \text{gcd}(x_1, \ldots, x_k)$ and $a \in \mathbb{Z}$. Therefore $H$ is generated by just one element $\gcd(x_1, \ldots, x_k)$ and is cyclic. Indeed, every subgroup of $\mathbb{Z}$ is of the form $n\mathbb{Z}$ which is isomorphic to $\mathbb{Z}$ itself. The quotient groups of $\mathbb{Z}$ are simply $\mathbb{Z}_n = \mathbb{Z}/n\mathbb{Z}$ which are the finite cyclic groups when $n \ne 0$. What happens when $n=0$?  Clearly, $\mathbb{Z}_0 = \mathbb{Z}$.

Now we look into subgroups and quotient groups of finite cyclic groups. Consider $\mathbb{Z}/n\mathbb{Z} = \lbrace \bar{0}, \bar{1}, \ldots, \overline{n-1} \rbrace$. The notation specifies that $\bar{k}$ is an equivalent class and is the set $\lbrace k, k+n, k+2n, \ldots \rbrace$. Let $H$ be a subgroup of $\mathbb{Z}/n\mathbb{Z}$. By Lagrange's theorem, $|H|$ must divide $n$. There are a couple of things to remember now. Firstly, every subgroup of a cyclic group is cyclic. Secondly, all cyclic groups of the same order are isomorphic to one another and to $\mathbb{Z}/k\mathbb{Z}$ where $k$ is the order. Thus, there is a unique subgroup of $\mathbb{Z}/k\mathbb{Z}$ of order $k$ for every $k$ that divides $n$. For example, consider $\mathbb{Z}/12 \mathbb{Z}$. The divisors of $12$ are $1$, $2$, $3$, $4$, $6$ and $12$. The subgroups are then *isomorphic* to $\lbrace \bar{0} \rbrace = \mathbb{Z}/\mathbb{Z}$, $\mathbb{Z}/2\mathbb{Z}$, $\mathbb{Z}/3\mathbb{Z}$, $\mathbb{Z}/4\mathbb{Z}$, $\mathbb{Z}/6\mathbb{Z}$ and $\mathbb{Z}/12\mathbb{Z}$. Precisely, the subgroups are

- $\lbrace \bar{0}$ 
- $\lbrace \bar{0}, \bar{6} \rbrace \cong \mathbb{Z}/2\mathbb{Z}$
- $\lbrace \bar{0}, \bar{4}, \bar{8} \rbrace \cong \mathbb{Z}/3\mathbb{Z}$
- $\lbrace \bar{0}, \bar{3}, \bar{6}, \bar{9} \rbrace \cong \mathbb{Z}/4\mathbb{Z}$
- $\lbrace \bar{0}, \bar{2}, \bar{4}, \bar{6}, \bar{8}, \bar{10} \rbrace \cong \mathbb{Z}/6\mathbb{Z}$
- $\mathbb{Z}/12\mathbb{Z}$

The treatment of quotient qroups is almost exactly the same once we realize that quotients of cyclic groups are cyclic. For example, what exactly is $\mathbb{Z}/12\mathbb{Z}/\lbrace \bar{0}, \bar{4}, \bar{8} \rbrace$? By Lagrange's theorem, it must be of order $4$ and hence isomorphic to $\mathbb{Z}/4\mathbb{Z}$.
