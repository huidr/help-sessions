...
Now we look into subgroups and quotient groups of finite cyclic groups. Consider $\mathbb{Z}_n = \lbrace \bar{0}, \bar{1}, \ldots, \overline{n-1} \rbrace$. The notation specifies that $\bar{k}$ is an equivalent class and is the set $\lbrace k, k+n, k+2n, \ldots \rbrace$. Let $H$ be a subgroup of $\mathbb{Z}_n$. By Lagrange's theorem, $|H|$ must divide $n$. There are a couple of things to remember now. Firstly, every subgroup of a cyclic group is cyclic. Secondly, all cyclic groups of the same order are isomorphic to one another and to $\mathbb{Z}_k$ where $k$ is the order. Thus, there is a unique subgroup of $\mathbb{Z}_n$ of order $k$ for every $k$ that divides $n$. For example, consider $\mathbb{Z}_{12}$.
